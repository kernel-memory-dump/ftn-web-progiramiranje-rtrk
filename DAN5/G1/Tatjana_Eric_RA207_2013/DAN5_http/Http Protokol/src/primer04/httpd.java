...
public class httpd {
	...
	static void sendResponse(String resource, OutputStream os, List<String> users)
		...
		if(resource.endsWith(".html")){
		...
		} else if(resource.startsWith("dodaj")) { // dodajKorisnika?ime=Pera
			String[] prviDeo=resource.split("\\?");
			String imeVrednost=prviDeo[1];
			String[] drugiDeo=resource.split("\\=");
			...
			ps.print(response);
			ps.flush();
		}else if(resource.startsWith("brisi")) { // dodajKorisnika?ime=Pera
			String[] prviDeo=resource.split("\\?");
			String imeVrednost=prviDeo[1];
			String[] drugiDeo=resource.split("\\=");
			String ime=drugiDeo[1];
	...
	static String browserResponse(List<String>users) {
