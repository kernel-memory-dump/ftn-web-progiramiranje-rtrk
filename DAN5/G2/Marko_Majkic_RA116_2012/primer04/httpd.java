...
public class httpd {
	...
	static void sendResponse(String resource, OutputStream os)
		...
		if(resource.endsWith(".html"))
		{
		...
		else if (resource.startsWith("add")) {
			
			int startIndex = resource.indexOf("ime=") + "ime=".length();
			int endIndex = resource.indexOf("&");
			String name = resource.substring(startIndex, endIndex);
			...
			Osoba osoba = new Osoba(name, tip);
			
			users.add(osoba);
		
			ps.println(browserResponse(null));
		}	
		else if (resource.startsWith("remove")) {
			String[] part1 = resource.split("\\?");
			String parametar = part1[1];
			String[] part2 = parametar.split("\\=");
			String name2 = part2[1];
			name2 = URLDecoder.decode(name2, "UTF-8");
			
			Osoba osoba = new Osoba(name2, null);
	...
	static private String browserResponse(String type) {
		...
		for (int i = 0; i < users.size(); i++) {
			...
			else if(tip.equals(type) || type.equals("All"))
			{				
				retVal += "<li>" + ime + ": " + tip +"</li>\n";
			}
		}
